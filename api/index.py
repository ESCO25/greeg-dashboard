...
@app.route("/api/reaction-roles", methods=["GET"])
@require_auth
def get_reaction_roles():
    d = get_db()
    panels = list(d["reaction_roles"].find({"guild_id": GUILD_ID}).sort("created_at", -1))
    result = []
    for p in panels:
        result.append({
            "id": str(p["_id"]),
            ...
            "image": p.get("image", ""),
            "footer": p.get("footer", ""),
            "unique_mode": p.get("unique_mode", False),
            "roles": p.get("roles", []),
...
@require_auth
def create_reaction_roles():
    """
    ينشئ لوحة رولات تفاعلية، يحفظها في MongoDB،
    ثم يضع أمر للبوت لإرساله وإضافة الريأكشنات.
    """
    data = request.get_json() or {}
    channel_id = data.get("channel_id", "")
    roles = data.get("roles", [])  # [{emoji, role_id}]
    if not channel_id:
        return jsonify({"error": "channel_id مطلوب"}), 400
    if not roles or not isinstance(roles, list):
        return jsonify({"error": "أضف رولاً واحداً على الأقل مع إيموجي"}), 400
    for r in roles:
        if not r.get("emoji") or not r.get("role_id"):
            return jsonify({"error": "كل صف يجب أن يحتوي إيموجي ورول"}), 400
    ...
    doc = {
        ...
        "channel_id": str(channel_id),
        "message_id": "",
        "name": data.get("name", "").strip() or data.get("title", "لوحة رولات"),
        "title": data.get("title", ""),
        "description": data.get("description", ""),
        "color": data.get("color", 0x5865f2),
        "image": data.get("image", ""),
        "footer": data.get("footer", ""),
        "unique_mode": bool(data.get("unique_mode", False)),
        "roles": [{"emoji": r["emoji"], "role_id": str(r["role_id"])} for r in roles],
        "status": "pending",
        "created_at": datetime.now(timezone.utc).isoformat(),
    }
    result = d["reaction_roles"].insert_one(doc)
    panel_id = str(result.inserted_id)

    cmd_id = queue_bot_command("send_reaction_panel", {
        "panel_id": panel_id,
        "channel_id": str(channel_id),
        "title": doc["title"],
    ...
    return jsonify({"success": True, "panel_id": panel_id, "command_id": cmd_id, "note": "تم إرسال الأمر للبوت"})

@app.route("/api/reaction-roles/<panel_id>", methods=["DELETE"])
@require_auth
def delete_reaction_roles(panel_id):
    d = get_db()
    try:
        panel = d["reaction_roles"].find_one({"_id": ObjectId(panel_id), "guild_id": GUILD_ID})
    except InvalidId:
        return jsonify({"error": "معرف غير صالح"}), 400
    if not panel:
        return jsonify({"error": "اللوحة غير موجودة"}), 404
    queue_bot_command("delete_reaction_panel", {
        "channel_id": panel.get("channel_id", ""),
        "message_id": panel.get("message_id", ""),
    })
    d["reaction_roles"].delete_one({"_id": ObjectId(panel_id)})
    return jsonify({"success": True})

# ── Contest (مشاركة في المسابقة) ────────────────────────────────
# ملاحظة: القيم الافتراضية هنا يجب أن تبقى مطابقة لـ DEFAULT_SETTINGS
# في cogs/contest.py حتى لا يختلف السلوك بين البوت والداشبورد.
...
@app.route("/api/contest/settings", methods=["GET"])
@require_auth
def get_contest_settings_api():
    d = get_db()
    doc = d["contest_settings"].find_one({"guild_id": GUILD_ID}, {"_id": 0}) or {}
    merged = CONTEST_DEFAULTS.copy()
    merged.update(doc)
    if not merged.get("questions"):
        merged["questions"] = CONTEST_DEFAULTS["questions"]
    return jsonify(merged)

@app.route("/api/contest/settings", methods=["POST"])
@require_auth
def update_contest_settings_api():
    d = get_db()
    data = request.get_json() or {}
    update = {}
    ...
    if "image_required" in data:
        update["image_required"] = bool(data["image_required"])

    if "upload_timeout" in data:
        try:
            update["upload_timeout"] = max(30, min(int(data["upload_timeout"]), 900))
        except (TypeError, ValueError):
            return jsonify({"error": "upload_timeout يجب أن يكون رقمًا"}), 400

    if "color" in data:
        try:
            c = data["color"]
            update["color"] = int(c, 16) if isinstance(c, str) else int(c)
        except (TypeError, ValueError):
            return jsonify({"error": "color غير صالح"}), 400

    if "questions" in data:
        qs = data["questions"]
        if not isinstance(qs, list) or not qs:
            return jsonify({"error": "أضف سؤالاً واحدًا على الأقل"}), 400
        if len(qs) > 5:
            return jsonify({"error": "الحد الأقصى 5 أسئلة (قيد من ديسكورد نفسه)"}), 400
        clean_qs = []
        for q in qs:
...
@app.route("/api/contest/panel", methods=["POST"])
@require_auth
def send_contest_panel():
    """
    ...
    settings = d["contest_settings"].find_one({"guild_id": GUILD_ID}) or {}
    panel_channel_id = panel_channel_id or settings.get("panel_channel_id")
    if not panel_channel_id:
        return jsonify({"error": "panel_channel_id مطلوب (حدد روم اللوحة أولاً)"}), 400
    if not settings.get("target_channel_id"):
        return jsonify({"error": "حدد روم استقبال المشاركات (target_channel_id) أولاً"}), 400

    d["contest_settings"].update_one(
        {"guild_id": GUILD_ID},
        {"$set": {"panel_channel_id": str(panel_channel_id)}},
        upsert=True
    )
...
def get_contest_entries():
    d = get_db()
    limit = min(int(request.args.get("limit", 50)), 200)
    entries = list(d["contest_entries"].find({"guild_id": GUILD_ID}).sort("created_at", -1).limit(limit))
    result = []
    for e in entries:
...
@require_auth
def delete_contest_entry(entry_id):
    d = get_db()
    try:
        entry = d["contest_entries"].find_one({"_id": ObjectId(entry_id), "guild_id": GUILD_ID})
    except InvalidId:
        return jsonify({"error": "معرف غير صالح"}), 400
    if not entry:
        return jsonify({"error": "المشاركة غير موجودة"}), 404

    if entry.get("channel_id") and entry.get("message_id"):
        queue_bot_command("delete_single_message", {
            "channel_id": entry["channel_id"],
            "message_id": entry["message_id"],
        })
    d["contest_entries"].delete_one({"_id": ObjectId(entry_id)})
    return jsonify({"success": True})
